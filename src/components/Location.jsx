...
export default function Location({ onLocationClick }) {
  ...
  return (
    <div className="bg-gray-200 pb-12 lg:w-[50%] lg:mx-auto lg:ml-[8%] md:w-full">
      <div className="px-4 py-8 max-w-[1400px] w-full  m-auto">
        <h3 className="text-3xl font-title  font-extrabold text-blue-700 text-center">
      ...
      {categories.map((category) => {
        ...
        return (
          <div key={category} className="px-4 py-4">
            <h4 className="text-2xl font-title  font-extrabold text-primary mb-2">
              {category} Locations
            </h4>
            <div className="font-body grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 place-items-center max-w-[1400px] w-full m-full gap-x-0 gap-y-0">
              {filteredLocations.map((city, i) => (
                <p key={i} className="w-full flex items-start">
                  <a href="#header">
                    <button
                      onClick={() => onLocationClick(city.area)}
                      className="font-body text-left py-2 px-4 bg-transparent border-2 border-transparent rounded-xl hover:shadow-xl hover:border-gray-300 transition-all duration-300 hover:font-semibold"
                    >
