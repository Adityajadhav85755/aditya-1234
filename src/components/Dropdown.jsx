    ...
    return (
        <div className="w-full flex flex-col gap-y-2">
            ...
            {items.map((item, i) => {
                return <div key={i} className="">
                    <button
                        className="w-full bg-gray-300 text-primary font-title font-semibold py-3 text-left px-4 relative rounded-lg rounded-bl-none rounded-br-none"
                        onClick={() => { openDropdown(i) }}
                    >
                        {item.Item}
                        <i className="fa-solid fa-angle-down absolute right-5 top-1/2 -translate-y-1/2"></i>
                    ...
                    >
                        ...
                        {item.options.map((item, j) => {
                            // {console.log(item)}
                            return <div key={j}
                                className=" py-2 px-4  font-body text-primary flex justify-between items-center"
                            >
                                <div className="font-body  overflow-hidden flex items-center p-1 gap-4">
                                    <div>

                                    {/* {console.log(item.src)} */}
                                    <img src={item.src} alt="" className="w-12 h-12 font-body border-primary border-2 p-1 rounded-md" 
                                     onClick={() => onImageClick(item.src)}
                                    />
                                    </div>
                                {item.name}
                                </div>
                                <span className="text-primary font-body border-primary border px-2 py-1 rounded-md cursor-pointer" onClick={item.quantity === 0 ? () => addItem(j, i) : undefined}> {item.quantity >= 1 ? <div className="flex gap-x-2 items-center">
                                    <div className="flex gap-x-2 items-center">
                                        <i className="fa-solid fa-minus" onClick={() => { decrementQuantity(item.quantity, i, j) }}></i>
                                        {item.quantity}
                                        {/* {()=>{incrementTotalItem(item.quantity)}} */}
